...
peekAnyChar :: Parser Char
peekAnyChar = Parser $ \buf err ok ->
    case buf of
        []   -> runParser (getMore >> anyChar) buf err ok
        c1:_ -> ok buf c1
...
-- | Equilvalent to takeWhile, but do not fail if the end of the buffer
-- is reached.
takeWhileToEnd :: (Char -> Bool) -> Parser [Char]
takeWhileToEnd predicate = Parser $ \buf err ok ->
    case span predicate buf of
        (b1, b2) -> ok b2 b1
